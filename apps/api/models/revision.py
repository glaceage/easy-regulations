...
ALLOWED_TRANSITIONS: dict[RevisionState, set[RevisionState]] = {
    ...
    # PUBLISHED is reachable only via publish_revision (which creates a PolicyVersion),
    # never through a bare state transition.
    RevisionState.PENDING_PUBLISH: {RevisionState.CANCELLED},
...
class Revision(Base, TimestampMixin):
    ...
    state: Mapped[RevisionState] = mapped_column(pg_enum(RevisionState), default=RevisionState.DRAFT)
